...
- Facts:
    ...
    - Mixing different code languages in the same file is a bad idea. Separte files for layout (HTML), behavior (JS), and styles (CSS). Keep related files close to each other. Modern browsers can multiplex a single TCP connection and load resources faster. Features like prefetch and preload streamline that process.
- Assumptions:
