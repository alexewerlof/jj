...
function toEl(x: JJ | Element): Element {
    return x instanceof JJ ? x.el : x
}

const jjCache = new WeakMap<Element, JJ>()

function jj(x: JJ | Element) {
    if (x instanceof JJ) {
        return x
    }

    if (x instanceof Element) {
        return jjCache.get(x) || new JJ(x)
    }

    throw TypeError(`Expecting a JJ or Element. Got ${x}`)
}
...
export class JJ {
    ...
    clone(deep: boolean) {
        return new JJ(this.el.cloneNode(deep))
    }
...
Object.assign(JJ.prototype, control, events)
...
attrShorthands.forEach(attrName => {
    JJ.prototype[attrName] = function (attrValue) {
...
eventShorthands.forEach(eventName => {
    JJ.prototype[eventName] = function (...handlers) {
